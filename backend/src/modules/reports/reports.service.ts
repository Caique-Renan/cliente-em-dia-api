...
export class ReportsService {
  async getOverview(companyId: string, period: Period) {
    ...
    const [
      ...
      // won: current state
      prisma.attendance.count({ where: { companyId, status: 'WON' } }),

      // lost: current state
      prisma.attendance.count({ where: { companyId, status: 'LOST' } }),
    ...
    const quotesMap = Object.fromEntries(
      quotesByStatus.map(q => [q.status, { count: q._count._all, valueCents: q._sum.totalValueCents ?? 0 }])
    );
    ...
    const acceptedInPeriod = await prisma.quote.aggregate({
      where: {
        companyId,
        status: 'ACCEPTED',
    ...
    const msgMap = Object.fromEntries(
      messagesByAction.map(m => [m.action, m._count._all])
    );
